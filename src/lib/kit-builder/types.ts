...
export interface KitItemPersonalization {
   ...
   */
  artworkUrl?: string;
  /** Render generated by the mockup service for this exact configuration. */
  generatedMockupUrl?: string;
  /** Palette selected for the artwork, distinct from the billable color count. */
  artworkColors?: string[];
  /** Unit sale price returned by the pricing RPC for `pricedQuantity`. */
  estimatedPrice?: number;
  /** Setup component returned by the pricing RPC, already including its commercial markup. */
  setupCost?: number;
  /** Authoritative amount charged for the complete priced quantity (includes minimum/setup rule). */
  totalPrice?: number;
  /** Exact quantity used by the RPC. A different quantity invalidates all persisted prices. */
...
/** Semantic constraints returned by the AI. Catalog facts are resolved locally. */
export interface KitAISuggestionBrief {
  kit_type: KitType;
  box_keywords: string[];
...
export interface KitAIComposition {
  id: string;
  name: string;
  narrative: string;
  kitType: KitType;
  ...
  items: KitItem[];
  unitPrice: number;
  fitStatus: 'compatible' | 'inconclusive';
}
