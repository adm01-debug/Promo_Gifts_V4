...
export const PublicMagazineToc = memo(
  ({ open, onOpenChange, magazine, pages, currentIndex, onGo, bookmarks }: Props) => {
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent
          side="right"
          className="w-[420px] max-w-[92vw] overflow-y-auto bg-neutral-950 text-white"
        >
          <SheetHeader>
            <SheetTitle className="text-white">Sumário</SheetTitle>
            <p className="text-xs text-white/60">{pages.length} páginas · toque para pular</p>
          </SheetHeader>
                ...
                <li key={p.pageId ?? p.index}>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between gap-2 bg-white/5 px-2 py-1.5 text-[10px] uppercase tracking-widest text-white/80">
                      <span className="tabular-nums">{String(p.index + 1).padStart(3, '0')}</span>
