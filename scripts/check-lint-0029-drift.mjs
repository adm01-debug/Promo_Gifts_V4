 ...
 *
 * Falha se surgir uma função `public.*` `SECURITY DEFINER` executável
 * por `authenticated` (Supabase lint 0029) que NÃO esteja documentada
 * em `.security/lint-0029-allowlist.json`.
 *
...
const ALLOWLIST_PATH = path.join(ROOT, '.security/lint-0029-allowlist.json');
...
function loadAllowlist() {
  if (!existsSync(ALLOWLIST_PATH)) {
    process.stderr.write(`[lint-0029] allowlist ausente: ${ALLOWLIST_PATH}\n`);
    process.exit(2);
  }
  const doc = JSON.parse(readFileSync(ALLOWLIST_PATH, 'utf8'));
  if (!Array.isArray(doc.functions)) {
    process.stderr.write(`[lint-0029] allowlist.functions inválida\n`);
    process.exit(2);
  }
...
async function main() {
  ...
  if (UPDATE) {
    ...
    process.stderr.write(
      `[lint-0029] allowlist atualizada: +${newFindings.length} / -${staleAllowlist.length}\n`,
    );
    process.exit(0);
  }
  ...
  if (newFindings.length) {
    problems.push(
      `❌ ${newFindings.length} finding(s) 0029 NÃO documentados na allowlist:\n` +
  ...
  if (staleAllowlist.length) {
    process.stderr.write(
      `⚠️  ${staleAllowlist.length} entrada(s) da allowlist não existem mais no DB:\n` +
        ...
        `\n   Rode: node scripts/check-lint-0029-drift.mjs --update-allowlist\n`,
    );
  }
  ...
  if (problems.length) {
    process.stderr.write('\n' + problems.join('\n\n') + '\n\n');
    process.stderr.write(
      'Para aceitar novos findings: adicione manualmente em .security/lint-0029-allowlist.json com um `reason` real,\n' +
        'OU rode `node scripts/check-lint-0029-drift.mjs --update-allowlist` e edite o motivo antes do commit.\n',
    );
    process.exit(1);
  }

  process.stderr.write(
    `✅ lint 0029: ${actual.length} finding(s) — todos documentados na allowlist.\n`,
  );
  process.exit(0);
}

main().catch((e) => {
  process.stderr.write(`[lint-0029] erro: ${e.stack || e.message}\n`);
  process.exit(2);
});
