 ...
 *
 * Falha se surgir uma função `public.*` com `SECURITY DEFINER` executável
 * por `anon` (bypass de auth) que NÃO esteja documentada em
 * `.security/secdef-anon-allowlist.json`.
 *
 * Por que importa: SECURITY DEFINER + EXECUTE para anon = qualquer requisição
 * não-autenticada executa a função com os privilégios do OWNER, ignorando
 * RLS e outras checagens. É o padrão exato de vazamento de dados relatado
 * em CVEs de Postgres/Supabase quando helpers admin são acidentalmente
 * expostos. O snapshot 2026-07-15 tem 0 violações → o gate trava regressões.
 *
...
const ALLOWLIST_PATH = path.join(ROOT, '.security/secdef-anon-allowlist.json');
...
async function main() {
  ...
  if (UPDATE) {
    ...
    for (const fn of newFindings) {
      merged.set(fn, { fn, reason: 'TODO: documentar motivo antes de aprovar PR (endpoint público?)' });
    }
    ...
    process.stderr.write(
      `[secdef-anon] allowlist atualizada: +${newFindings.length} / -${staleAllowlist.length}\n`,
    );
    process.exit(0);
  }
  ...
  if (newFindings.length) {
    problems.push(
      `🚨 ${newFindings.length} SECURITY DEFINER exposta(s) para anon SEM revisão:\n` +
  ...
  if (staleAllowlist.length) {
    process.stderr.write(
      `⚠️  ${staleAllowlist.length} entrada(s) da allowlist não existem mais no DB:\n` +
        ...
        `\n   Rode: node scripts/check-secdef-anon-drift.mjs --update-allowlist\n`,
    );
  }
  ...
  if (problems.length) {
    process.stderr.write('\n' + problems.join('\n\n') + '\n\n');
    process.stderr.write(
      'Correção padrão (recomendada): revogar a exposição a anon.\n' +
        '  REVOKE EXECUTE ON FUNCTION public.<nome>(<args>) FROM anon, PUBLIC;\n\n' +
        'Se a função DEVE mesmo ser executável por anon (endpoint público via token, etc.),\n' +
        'adicione em .security/secdef-anon-allowlist.json com um `reason` real,\n' +
        'OU rode `node scripts/check-secdef-anon-drift.mjs --update-allowlist` e edite o motivo.\n',
    );
    process.exit(1);
  }

  process.stderr.write(
    `✅ secdef-anon: ${actual.length} finding(s) — todos documentados na allowlist.\n`,
  );
  process.exit(0);
}

main().catch((e) => {
  process.stderr.write(`[secdef-anon] erro: ${e.stack || e.message}\n`);
  process.exit(2);
});
