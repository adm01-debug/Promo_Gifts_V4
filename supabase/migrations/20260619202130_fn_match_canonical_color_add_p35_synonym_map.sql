...
-- ============================================================
-- Migration: fn_match_canonical_color + P3.5 color_synonym_map
-- Adiciona consulta à tabela color_synonym_map entre P3 e P4
-- Também corrige lacunas na P4.5 inline: VINHO, FUMÊ, BRONZE
-- ============================================================
...
BEGIN
  IF v_name_norm IS NULL AND v_hex_norm IS NULL THEN RETURN NULL; END IF;

  -- P1: Match exato por nome em color_variations
  SELECT cv.id INTO v_id FROM public.color_variations cv
  WHERE cv.is_active = TRUE AND v_name_norm IS NOT NULL AND UPPER(TRIM(cv.name)) = v_name_norm
  ORDER BY cv.sort_order NULLS LAST LIMIT 1;
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;

  -- P2: Match exato por hex em color_variations
  SELECT cv.id INTO v_id FROM public.color_variations cv
  WHERE cv.is_active = TRUE AND v_hex_norm IS NOT NULL AND UPPER(TRIM(cv.hex_code)) = v_hex_norm
  ORDER BY cv.sort_order NULLS LAST LIMIT 1;
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;
  ...
  WHERE sc.is_active = TRUE AND v_name_norm IS NOT NULL AND UPPER(TRIM(sc.name)) = v_name_norm
  ORDER BY ce.confidence_score DESC NULLS LAST, cv.sort_order NULLS LAST LIMIT 1;
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;

  -- P3.5: color_synonym_map — lookup direto por nome (novo: antes de hex e sinônimos)
  SELECT csm.canonical_color_id INTO v_id
  FROM public.color_synonym_map csm
  WHERE v_name_norm IS NOT NULL
    AND UPPER(TRIM(csm.supplier_color_name)) = v_name_norm
  ORDER BY csm.confidence DESC NULLS LAST
  LIMIT 1;
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;
  ...
  JOIN public.color_equivalences ce ON ce.supplier_color_id = sc.id AND ce.is_active = TRUE
  JOIN public.color_variations cv ON cv.id = ce.promo_variation_id AND cv.is_active = TRUE
  WHERE sc.is_active = TRUE AND v_hex_norm IS NOT NULL AND UPPER(TRIM(sc.hex_code)) = v_hex_norm
  ORDER BY ce.confidence_score DESC NULLS LAST, cv.sort_order NULLS LAST LIMIT 1;
  IF v_id IS NOT NULL THEN RETURN v_id; END IF;

  -- P4.5: Tabela de sinônimos universais inline — v4 (inclui VINHO, FUMÊ, BRONZE + acento)
  IF v_name_norm IS NOT NULL THEN
    ...
    FROM (VALUES
      ...
      ('AQUAMARINE','fc050f73-79fc-4c00-bcee-f4b2cb8e17da'::uuid),
      ('KHAKI','17108bfc-9fed-4c71-bb68-ee3db0dfc681'::uuid),
      ('MINT','81991704-b57b-42ae-90a5-99b0f1521f49'::uuid),
      ('ROSE GOLD','9dc29020-e932-4a2a-bdc6-d13ddf5e31a5'::uuid),
      -- Hífen
      ...
      -- Com acento
      ('PETRÓLEO','0904add6-5ac9-43e5-a025-10d561e27f2a'::uuid),
      ('PETROLEO','0904add6-5ac9-43e5-a025-10d561e27f2a'::uuid),
      ('ÂMBAR','d7d27bf3-176c-4238-b7a2-77719fe63f59'::uuid),
      ('LILÁS','abc41b40-b938-4e39-bb16-12ee05578535'::uuid),
      ('PÚRPURA','56dd484f-b17f-47d2-a8ae-f6612069227d'::uuid),
      -- Variações comuns
      ...
      ('FUMÊ','35995f65-4e96-42f6-9a05-1e725d3a88f3'::uuid),   -- ← NOVO: com acento
      ...
      ('MENTA','81991704-b57b-42ae-90a5-99b0f1521f49'::uuid),
      ...
      ('VINHO','8e9984bf-0139-42ed-aa2a-26eb6a121aed'::uuid),    -- ← NOVO
      ('BRONZE','b4feaacd-2442-4597-bf55-c9c36ea61756'::uuid)    -- ← NOVO: Bronze → Prata (metálico)
    ) AS syn(alias, variation_id)
    WHERE syn.alias = v_name_norm LIMIT 1;
    IF v_id IS NOT NULL THEN RETURN v_id; END IF;
  END IF;

  -- P5: Match parcial por nome via color_groups (fallback)
  SELECT cv.id INTO v_id
  FROM public.color_groups cg
  JOIN public.color_variations cv ON cv.color_group_id = cg.id AND cv.is_active = TRUE
  WHERE cg.is_active = TRUE AND v_name_norm IS NOT NULL
    AND v_name_norm LIKE '%' || UPPER(TRIM(cg.name)) || '%'
  ORDER BY CASE WHEN UPPER(TRIM(cv.name)) = UPPER(TRIM(cg.name)) THEN 0 ELSE 1 END,
    cv.sort_order NULLS LAST LIMIT 1;
