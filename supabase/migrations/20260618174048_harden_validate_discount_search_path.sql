...
-- Segurança (cubic P1): fn_quotes_validate_discount é SECURITY DEFINER mas não tinha
-- search_path explícito → vulnerável a hijacking de resolução de objetos via
-- search_path da sessão. Adiciona SET search_path TO 'pg_catalog','public' (padrão
-- do repo, idêntico a fn_quotes_calc_real_values). Lógica do corpo inalterada.
