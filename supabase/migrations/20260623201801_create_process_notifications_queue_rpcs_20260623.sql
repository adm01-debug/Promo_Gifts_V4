...
-- BUG-4 FIX: Criar RPCs ausentes que causam 500 em process-queue
...
-- 
-- Design:
--   process_notifications_queue: retorna notificações não lidas, atômico com
--   limpeza de expiradas (sem expires_at na tabela; implementamos select simples).
--   confirm_notifications_dispatched: marca como lidas as notificações despachadas.
...
-- ============================================================
-- RPC 1: process_notifications_queue
-- Retorna notificações não lidas em ordem FIFO, limite configurável.
-- SECURITY DEFINER + search_path para evitar schema injection.
-- ============================================================
...
-- ============================================================
-- RPC 2: confirm_notifications_dispatched
-- Marca como lidas as notificações cujos IDs foram despachados.
-- Idempotente: re-marcar já-lidas é no-op.
-- ============================================================
...
-- Garantir permissões para service_role (bypass RLS via SECURITY DEFINER)
