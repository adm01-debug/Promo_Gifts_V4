...
-- =====================================================================
-- MIGRATION: Melhoria 2 + 3 combinadas na mesma view
-- 2026-06-22 — APLICADO
--
-- Melhoria 2: Cap score CRÍTICO a 99.99 (RUPTURA=100 único e inequívoco)
-- Melhoria 3: Winsorização EMA — corrige 4 outliers XBZ com stock<=10 e
--             EMA > 500× stock (bug de oscilação intraday no sync XBZ)
-- =====================================================================
...
WITH ema_calc AS (
  SELECT
    ...
    sv.days_to_stockout,
    /*
     * EMA α≈0.3 com WINSORIZAÇÃO AUTOMÁTICA:
     * Se stock <= 10 e EMA > 500× stock → dado corrompido por oscilação intraday
     * Nesses casos: EMA = floor 0.01 (tratamos como sem histórico confiável)
     * Resultado: cobertura_dias = stock/0.01 = stock*100 → classificação conservadora
     */
    CASE
      WHEN sv.current_stock <= 10
        AND GREATEST(
              0.01,
              CASE
                WHEN sv.avg_daily_depletion_7d  > 0 AND sv.avg_daily_depletion_30d > 0
                  THEN sv.avg_daily_depletion_7d*0.50 + sv.avg_daily_depletion_30d*0.35
                ...
                ELSE 0.01
              END
            ) > NULLIF(sv.current_stock, 0) * 500
        THEN 0.01  -- outlier detectado: zeramos EMA, item fica como slow-mover
      ELSE
        GREATEST(
          0.01,
          CASE
    ...
    END::numeric(10,4)                                                         AS ema_diaria
...
computed AS (
  SELECT
    e.vss_id, e.variant_id, e.supplier_id, e.ema_diaria,
    e.velocity_trend, e.active_days_30d,
    s.name                                                                     AS supplier_name,
    vss.supplier_sku,
    COALESCE(vss.is_preferred, FALSE)                                          AS is_preferred,
    (COALESCE(vss.stock_main_warehouse, 0) + COALESCE(vss.stock_other_warehouses, 0)) AS stock_total,
...
classified AS (
  SELECT c.*,
    ...
    END                                                                        AS prioridade,
    /*
     * score_composto com DOIS FIXES:
     * 1. RUPTURA=100 único: CRÍTICO capado a 99.99 (LEAST 99.99)
     * 2. LEAST 100 no outer wrapper previne overflow
     */
    LEAST(100, GREATEST(0,
      CASE
        WHEN c.stock_total = 0
          THEN 100.0   -- RUPTURA: score=100 EXCLUSIVO
        WHEN c.cobertura_dias < c.lead_time_efetivo
          THEN LEAST(99.99,  -- CRÍTICO: score máx 99.99
                75.0 + 25.0 * GREATEST(0.0,
                  1.0 - COALESCE(c.cobertura_dias, 0) / NULLIF(c.lead_time_efetivo::numeric, 0)))
...
-- Recriar todos os índices (DROP CASCADE removeu)
...
CREATE INDEX idx_mv_stock_rupture_alert_non_ok_prio_score
  ON public.mv_stock_rupture_alert (is_preferred, prioridade ASC, score_composto DESC)
  WHERE nivel_alerta <> 'OK';

-- Grants
GRANT SELECT ON public.mv_stock_rupture_alert TO anon, authenticated, service_role;
REVOKE INSERT, UPDATE, DELETE, TRUNCATE ON public.mv_stock_rupture_alert FROM anon, authenticated;

-- Recriar função (DROP CASCADE removeu)
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT m.nivel_alerta::text, m.prioridade::integer,
    COUNT(DISTINCT m.variant_id), ROUND(AVG(m.cobertura_dias)::numeric,1),
    ROUND(MIN(m.cobertura_dias)::numeric,1)
  FROM public.mv_stock_rupture_alert m
  WHERE (NOT p_preferred_only OR m.is_preferred = true)
  GROUP BY m.nivel_alerta, m.prioridade ORDER BY m.prioridade;
$$;

GRANT EXECUTE ON FUNCTION public.fn_ema_kpi_by_level(boolean) TO anon, authenticated, service_role;
