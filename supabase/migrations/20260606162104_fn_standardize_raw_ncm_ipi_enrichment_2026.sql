-- Adiciona enriquecimento de ipi_rate via ncm_codes após normalização do NCM.
-- Só enriquece quando supplier não forneceu ipi_rate (IS NULL) — conservador.
-- ipi=0 é considerado dado válido do fornecedor (preservado).
...
DECLARE
  ...
  v_ncm_ipi  numeric;   -- ← Melhoria 5: enriquecimento via ncm_codes
  v_cols text[] := ARRAY[
...
BEGIN
  ...
  LOOP
    -- ── Strip do prefixo JSONPath '$.campo' → 'campo' (E'' para escape correto) ──
    v_path := NULLIF(
                regexp_replace(
  ...
  -- ── NCM: normalizar + suporte Taric (SPOT) ────────────────────────────────
  ...
  IF v_ncm IS NOT NULL THEN
    ...
    -- ── Melhoria 5: enriquecer ipi_rate via ncm_codes quando supplier não forneceu ──
    -- Conservador: só enriquece quando ipi_rate está ausente (IS NULL na v_assigns).
    -- ipi=0 do fornecedor é preservado como dado válido.
    IF v_assigns->>'ipi_rate' IS NULL THEN
      BEGIN
        v_ncm_ipi := public.fn_get_ncm_ipi_rate(v_ncm);
        IF v_ncm_ipi IS NOT NULL THEN
          v_assigns := v_assigns || jsonb_build_object('ipi_rate', v_ncm_ipi::text);
          v_errs := v_errs || jsonb_build_object(
            'field','ipi_rate','stage','ncm_enrichment',
            'msg','ipi_enriquecido_via_ncm_codes','ncm',v_ncm,'ipi',v_ncm_ipi
          );
        END IF;
