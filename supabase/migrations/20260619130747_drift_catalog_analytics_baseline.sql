...
SELECT
  ...
  ((COALESCE(total_depleted_30d, 0) < 5) AND (COALESCE(total_current_stock, 0) > 100)) AS is_stagnant,
  ((COALESCE(total_depleted_30d, 0) < 5) AND (COALESCE(total_current_stock, 0) > 500)) AS is_negotiation_opportunity,
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname='idx_mv_product_intelligence_product_id_unique' AND schemaname='analytics') THEN
    EXECUTE 'CREATE UNIQUE INDEX idx_mv_product_intelligence_product_id_unique ON analytics.mv_product_intelligence (product_id)';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname='idx_mv_product_intelligence_abc' AND schemaname='analytics') THEN
    EXECUTE 'CREATE INDEX idx_mv_product_intelligence_abc ON analytics.mv_product_intelligence (abc_classification)';
  END IF;
END;
$$;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name='fn_get_product_intelligence_all' AND routine_schema='public') THEN
    REVOKE EXECUTE ON FUNCTION public.fn_get_product_intelligence_all() FROM PUBLIC;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.routines WHERE routine_name='fn_get_all_leaf_categories' AND routine_schema='public') THEN
...
-- ═══════════════════════════════════════════════════════════════════
-- 4. pg_cron jobs de refresh
-- ═══════════════════════════════════════════════════════════════════
DO $$
BEGIN
  PERFORM cron.unschedule(jobid) FROM cron.job
  WHERE jobname IN ('refresh-mv-product-leaf-category','refresh-analytics-mv-product-intelligence','refresh-category-ancestors');

  PERFORM cron.schedule('refresh-mv-product-leaf-category','0 */4 * * *',
    'REFRESH MATERIALIZED VIEW CONCURRENTLY public.mv_product_leaf_category');
  PERFORM cron.schedule('refresh-analytics-mv-product-intelligence','30 2 * * *',
    'REFRESH MATERIALIZED VIEW CONCURRENTLY analytics.mv_product_intelligence');
  PERFORM cron.schedule('refresh-category-ancestors','0 1 * * *',
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname='mv_stock_velocity_pk' AND schemaname='analytics') THEN
    EXECUTE 'CREATE UNIQUE INDEX mv_stock_velocity_pk ON analytics.mv_stock_velocity (variant_supplier_source_id)';
  END IF;
END;
$$;
;
