...
-- ════════════════════════════════════════════════════════════
-- BUG REAL DETECTADO em fn_system_health_summary():
-- 
-- ERROR: cannot set parameter "role" within security-definer function
-- CAUSE: fn_system_health_summary() é SECURITY DEFINER e chama
--   fn_run_smoke_tests() que internamente usa EXECUTE 'SET LOCAL ROLE...'
--   / 'RESET role' — proibido em contexto SECURITY DEFINER (PG restriction).
--
-- FIX: Mudar fn_system_health_summary para SECURITY INVOKER
--   (não precisa de SECURITY DEFINER — é função de monitoramento
--    que lê dados com as permissões do chamador).
--   Substituir chamada a fn_run_smoke_tests() por query direta
--   simples nos checks de health, evitando o SET ROLE proibido.
-- ════════════════════════════════════════════════════════════
...
LANGUAGE plpgsql
STABLE
-- SECURITY INVOKER: chamador usa suas próprias permissões.
...
DECLARE
  ...
  v_fav_lists       bigint;
  -- Smoke test via contagem simples (não chama fn_run_smoke_tests para
  -- evitar o RESET role que é proibido em contexto SECURITY DEFINER)
  v_smoke_pass      int := 0;
  v_smoke_fail      int := 0;
  v_smoke_total     int;
BEGIN
  -- Verifica smoke tests via checks diretos (evita fn_run_smoke_tests + RESET role)
  -- Os checks mais críticos que podemos verificar sem SET ROLE:
  ...
  -- Check 2: Colunas GENERATED existem
...
-- Regrant (SECURITY INVOKER ainda precisa de GRANT EXECUTE)
...
COMMENT ON FUNCTION public.fn_system_health_summary() IS
  ...
  'fn_run_smoke_tests() usa RESET role que é proibido em SECURITY DEFINER. '
  'Retorna JSON com health checks internos diretos + métricas do sistema. '
  'Para smoke tests completos, use: SELECT * FROM fn_run_smoke_tests() diretamente.';
