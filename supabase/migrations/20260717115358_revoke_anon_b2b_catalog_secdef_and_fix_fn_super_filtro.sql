-- Migration 041a: Fix fn_super_filtro cost_price redaction + fn_get_category_breadcrumb SECURITY INVOKER
...
-- Make fn_get_category_breadcrumb SECURITY INVOKER (anon already has SELECT on categories)
...
-- Migration 041b: Revoke anon EXECUTE from B2B catalog SECURITY DEFINER functions
DO $$
DECLARE
  r         RECORD;
  v_ok      int := 0;
...
BEGIN
  FOR r IN
    ...
    WHERE n.nspname = 'public' AND p.proname = ANY(v_target) AND p.prokind = 'f'
  ...
  LOOP
    IF has_function_privilege('anon', r.oid, 'EXECUTE') THEN
      BEGIN
        ...
        v_ok := v_ok + 1;
        RAISE NOTICE '✓ [041] REVOKE EXECUTE ON %(%) FROM anon', r.proname, r.args;
      EXCEPTION WHEN OTHERS THEN
        v_fail := v_fail + 1;
        RAISE WARNING '✗ [041] Could not revoke %(%): %', r.proname, r.args, SQLERRM;
      END;
    ELSE
      v_skip := v_skip + 1;
    END IF;
  END LOOP;
  RAISE NOTICE '[041] Summary: revoked=%, skipped=%, failed=%', v_ok, v_skip, v_fail;
END;
$$;;
