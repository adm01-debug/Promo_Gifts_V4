...
BEGIN
  ...
  -- (e) NOVO: product_physical — peso + dimensões (gap-fill, COALESCE)
  ...
  -- (f) NOVO: product_properties — cores, gravação, vídeo, galeria (URLs)
  INSERT INTO public.product_properties (product_id, property_code, property_value, source, raw_value)
  ...
  CROSS JOIN LATERAL (VALUES
    ...
    ('site_imagens_urls',
        CASE WHEN jsonb_array_length(COALESCE(s.images, '[]'::jsonb)) > 0
             THEN s.images::text ELSE NULL END)
  ) AS kv(code, val)
  WHERE kv.val IS NOT NULL AND btrim(kv.val) <> ''
  ON CONFLICT (product_id, property_code) DO UPDATE
    SET property_value = EXCLUDED.property_value, raw_value = EXCLUDED.raw_value,
  ...
  -- (g) NOVO: xbz_gallery_staging — enfileira imagens para Cloudflare
  --     Constraint: uq_staging_codigo_d (codigo_amigavel, d_number) → ON CONFLICT DO NOTHING
  --     Limite: só imagens 0-9 (d_number 1-10)
  INSERT INTO public.xbz_gallery_staging
    (codigo_amigavel, d_number, url_original, product_id, status)
  SELECT DISTINCT
  ...
  ON CONFLICT (codigo_amigavel, d_number) DO NOTHING;
  GET DIAGNOSTICS v_imgs = ROW_COUNT;

  -- (h) NOVO: product_videos — YouTube via site
  INSERT INTO public.product_videos (
    product_id, organization_id, source_youtube_id, url_original,
    is_primary, source_supplier, is_active
  )
  SELECT s.product_id, v_org_id, s.video_embed_id, s.video_watch_url,
         true, 'xbz_site', true
  FROM _scope s
  WHERE s.has_video = true AND s.video_embed_id IS NOT NULL AND btrim(s.video_embed_id) <> ''
    AND NOT EXISTS (
      SELECT 1 FROM public.product_videos pv2
      WHERE pv2.product_id = s.product_id AND pv2.source_youtube_id = s.video_embed_id
    );
