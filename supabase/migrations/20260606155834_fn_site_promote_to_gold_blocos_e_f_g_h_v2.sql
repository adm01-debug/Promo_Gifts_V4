...
BEGIN
  ...
  --     + URLs das imagens (aguardando upload Cloudflare para product_images)
  INSERT INTO public.product_properties (product_id, property_code, property_value, source, raw_value)
  ...
  CROSS JOIN LATERAL (VALUES
    ...
    -- Galeria de imagens (URLs originais aguardando Cloudflare)
    ('site_imagens_urls',
  ...
  -- (g) NOVO: xbz_gallery_staging — garante que todas as imagens estão enfileiradas
  --     (o worker Cloudflare processa e promove para product_images com cloudflare_image_id)
  INSERT INTO public.xbz_gallery_staging (
    supplier_id, product_id, codigo_amigavel, d_number, url_original, status
  )
  SELECT DISTINCT
    s.supplier_id,
    s.product_id,
