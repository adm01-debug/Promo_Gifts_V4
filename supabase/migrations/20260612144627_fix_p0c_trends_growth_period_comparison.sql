...
BEGIN
  ...
  -- Período anterior (mesma duração, janela imediatamente anterior)
  SELECT COUNT(*)::bigint, COUNT(DISTINCT product_id)::bigint
  ...
  SELECT jsonb_build_object(
    'totals', jsonb_build_object(
      'views',           v_views_now,
      'searches',        v_searches_now,
      'unique_terms',    v_terms_now,
      'unique_products', v_prods_now,
      -- Growth: NULL quando sem dados anteriores (honesto, não inventado)
      ...
      -- Contexto do período anterior (para debug/UI)
      'prev_period_views', v_views_prev,
      'prev_period_searches', v_searches_prev
    ),
    -- ✅ P0-A: cast ::bigint para ordenação numérica correta
    ...
    'top_searches', COALESCE((
      ...
      FROM (
        ...
        FROM public.search_analytics
        WHERE created_at >= v_since
          -- Excluir sentinels -1 do count de zero_results
          AND results_count != -1
  ...
  IF p_user_id IS NOT NULL THEN
    INSERT INTO public.ai_insights_cache (
      ...
      'postgres_aggregation', 0, 0, 0, NOW(), NOW() + INTERVAL '23 hours'
    )
    ON CONFLICT (user_id, function_name, cache_key)
    DO UPDATE SET payload=EXCLUDED.payload,
      created_at=NOW(), expires_at=NOW()+INTERVAL '23 hours';
  END IF;
