-- UNIFICAÇÃO MEDALLION — Fase 7: adapta a cadeia legada "staging" para Bronze→Silver (de-para).
-- Antes: fn_process_staging_batch → fn_process_all_staged_products → fn_process_staged_product → Gold direto
-- (+ ramo de variantes inexistente). Agora: delega ao pipeline canônico produtos_padronizacao*.
...
BEGIN
  IF p_supplier_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'supplier_id_obrigatorio');
  END IF;
  -- ADAPTADO 2026-06-05: Bronze→Silver via pipeline canônico. Promoção Silver→Gold = cron oficial.
