-- UNIFICAÇÃO MEDALLION — Fase 4/4: aposenta o Silver legado (sem dropar).
DO $$
DECLARE
  v_msg_fn text := 'DEPRECATED 2026-06-05: Silver legado (medallion 001). Substituido pelo pipeline de-para oficial (produtos_padronizacao via fn_standardize_supplier + fn_promote_supplier). Sem chamadores. Candidato a DROP em follow-up.';
  v_msg_tb text := 'DEPRECATED 2026-06-05: tabela do Silver legado (medallion 001). Fonte canonica da Silver passou a ser produtos_padronizacao(+_variantes). Mantida para auditoria; candidata a DROP em follow-up.';
BEGIN
