-- ============================================================================
-- product_images :: torna fn_resync_product_media DETERMINÍSTICA e IDEMPOTENTE
-- O trigger original é não-determinístico em empates de (prioridade, is_primary,
-- display_order). Aqui adicionamos desempate estável (created_at, cloudflare_image_id)
-- em TODAS as ordenações, garantindo que execuções repetidas não causem churn.
-- A 1ª execução por produto pode normalizar a ordem de empates (cosmético, mesmo
-- conjunto de URLs); execuções seguintes são no-op.
-- ============================================================================
