-- BLOCO 1/6 — Observabilidade de sincronização Cloudflare Images
-- Aditivo e idempotente. Não remove/renomeia nada (CLAUDE.md REGRA #2 respeitada).
...
-- Coluna derivada (não exige backfill nem trigger): classifica a convenção do ID.
ALTER TABLE public.product_images
...
COMMENT ON COLUMN public.product_images.cf_id_scheme      IS 'Convencao do cloudflare_image_id (derivada): legacy_cor|dn|main_gal|slug_ts|synthetic|hash_legacy|uuid|seq';;
