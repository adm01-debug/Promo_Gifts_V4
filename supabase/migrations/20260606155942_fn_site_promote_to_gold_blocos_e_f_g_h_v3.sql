...
BEGIN
  ...
  -- (f) NOVO: product_properties — cores, gravação, vídeo, imagens (URLs)
  INSERT INTO public.product_properties (product_id, property_code, property_value, source, raw_value)
  ...
  -- (g) NOVO: xbz_gallery_staging — garante todas as imagens enfileiradas
  --     (worker Cloudflare processa e promove para product_images com cloudflare_image_id)
  --     Colunas: codigo_amigavel, d_number (1-10), url_original, product_id, status
  INSERT INTO public.xbz_gallery_staging
    (codigo_amigavel, d_number, url_original, product_id, status)
  SELECT DISTINCT
    s.supplier_reference,
    LEAST((img->>'ordem')::integer + 1, 10),
    img->>'url_origem',
    s.product_id,
    'discovered'
  FROM _scope s
  ...
  WHERE (img->>'url_origem') IS NOT NULL
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.xbz_gallery_staging gs
      WHERE gs.product_id   = s.product_id
        AND gs.url_original = img->>'url_origem'
    );
  ...
  SELECT s.product_id, v_org_id, s.video_embed_id,
         s.video_watch_url, true, 'xbz_site', true
  FROM _scope s
