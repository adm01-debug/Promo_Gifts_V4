...
-- Fix v3: usar DROP CASCADE para remover dependências, depois recriar tudo
-- Primeiro: listar dependências para recriar depois
-- Dependências identificadas: get_catalog_bestseller_page(text,integer,integer)
...
-- Recriar a função dependente (get_catalog_bestseller_page)
-- Primeiro verificar se ela depende de colunas específicas da view
-- ou apenas do tipo v_products_public

-- Nota: CASCADE dropa automaticamente get_catalog_bestseller_page
-- Esta função deve ser recriada. Verificar sua definição primeiro:
DO $$
DECLARE
  fn_def text;
BEGIN
  ...
  FROM pg_proc
  WHERE proname = 'get_catalog_bestseller_page'
  LIMIT 1;
  IF fn_def IS NULL THEN
    RAISE NOTICE 'get_catalog_bestseller_page: função não encontrada (ok se nunca existiu)';
  ELSE
    RAISE NOTICE 'AVISO: get_catalog_bestseller_page foi dropada por CASCADE e precisa ser recriada';
  END IF;
END;
$$;
