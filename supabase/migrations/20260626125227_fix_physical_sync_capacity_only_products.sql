-- Melhoria 2/4 (complemento) — capacity-only products.
-- O early-return ignorava linhas cujo único dado físico é capacity_ml.
-- fix_version=2026-06-26_physical_capacity_only
...
BEGIN
    ...
    -- Nada útil para copiar (agora considera capacity_ml também)
    IF v_weight_g IS NULL AND v_height_cm IS NULL AND v_width_cm IS NULL
       AND v_length_cm IS NULL AND v_diam_cm IS NULL AND v_cap_ml IS NULL THEN
        RETURN FALSE;
    END IF;
...
-- Backfill dos portadores de capacidade (inclui os 8 capacity-only).
DO $$
DECLARE r record; BEGIN
  FOR r IN SELECT id FROM products WHERE NULLIF(capacity_ml,0) IS NOT NULL
