...
-- PASSO 1: Atualiza fn_generate_market_insights_cache (remove tokens_input/output do INSERT)
...
BEGIN
  ...
  -- UPSERT atomico — tokens_input/tokens_output removidos (campos descontinuados)
...
-- PASSO 2: Atualiza fn_generate_trends_insights (remove tokens_input/output do INSERT)
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
    -- tokens_input/tokens_output removidos (campos descontinuados)
    INSERT INTO public.ai_insights_cache (
      ...
      'postgres_aggregation', 0, NOW(), NOW() + INTERVAL '6 hours'
    )
    ON CONFLICT (user_id, function_name, cache_key)
...
-- PASSO 3: Drop seguro das colunas mortas (ambas as funções ja nao as referenciam)
