...
BEGIN
    ...
    SELECT
        ...
        d.q_enrichment_type,
        -- ⭐ Copywriting config hierárquica (usa get_copywriting_config existente com fallback)
        public.get_copywriting_config(COALESCE(p.main_category_id, p.category_id)),
        -- ⭐ Raw data snapshot do Bronze: 18 campos para enriquecer o prompt
        (
...
COMMENT ON FUNCTION public.fn_dequeue_ai_enrichment(INTEGER, TEXT, TEXT) IS
    'v2 — Dequeue enriquecido. Retorna copywriting_config (hierárquica+fallback) e raw_data_snapshot (18 campos Bronze). Elimina 2-3 HTTP calls extras no n8n.';
;
