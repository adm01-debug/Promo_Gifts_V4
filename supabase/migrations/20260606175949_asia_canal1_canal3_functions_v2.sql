...
-- ================================================================
-- Migration: asia_canal1_canal3_functions_v2
-- 1. fn_ingest_asia_product: suporte ao novo Canal 1 (asia.ajung.site)
--    • COALESCE('sku', 'referencia') na variação → compatível com novo e legado
--    • Mapeamento de dimensoes_cm (objeto) e peso_kg
--    • embalagem, var_capacidade, var_volume
--    • content_hash explícito no INSERT
--    • source_channel='n8n_workflow', endpoint correto
-- 2. fn_upsert_asia_wp_batch: Canal 3 (WP hg/products → site_data)
-- ================================================================
...
BEGIN
  ...
  -- DISTINCT ON (slug) mantém 1ª ocorrência (resolve slug duplicado)
  ...
  LOOP
    -- -------------------------------------------------------
    -- CHAVE NATURAL: Canal 1 NOVO usa 'sku' (ex: CAD005-AZ)
    --               Canal 1 LEGADO usava 'referencia'
    -- -------------------------------------------------------
    v_var_ref := COALESCE(
    ...
    ON CONFLICT (supplier_id, supplier_sku) DO UPDATE SET
      ...
      content_hash       = v_chash,
      -- Só marca pending se o conteúdo mudou
      ...
      -- Só marca stock pending se o estoque mudou
...
-- Canal 3: recebe array de produtos do WP hg/products e grava
-- em site_data das linhas correspondentes no Bronze
-- ================================================================
...
BEGIN
  ...
  LOOP
    ...
    -- Canal 3 WP hg/products usa 'sku' = referência do produto-pai (ex: "CAD005")
    v_parent_ref := NULLIF(TRIM(v_item->>'sku'), '');
    CONTINUE WHEN v_parent_ref IS NULL;

    -- Remove 'variations' para não inflar site_data com dados redundantes de variante
    ...
    -- identifica pelo raw_data->>'referencia' que é gravado pelo Canal 1
