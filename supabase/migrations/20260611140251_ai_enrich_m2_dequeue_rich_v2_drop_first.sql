...
-- Drop necessário para alterar o tipo de retorno
...
BEGIN
    ...
    SELECT
        ...
        d.enrichment_type,
        -- ⭐ Copywriting config hierárquica com fallback automático
        public.get_copywriting_config(COALESCE(p.main_category_id, p.category_id)),
        -- ⭐ Raw data snapshot do Bronze (20 campos úteis para o prompt)
        (
...
COMMENT ON FUNCTION public.fn_dequeue_ai_enrichment(INTEGER, TEXT, TEXT) IS
    'v2 — Dequeue com dados ricos para o n8n:
     + copywriting_config: JSON via get_copywriting_config (hierarquia + fallback).
     + raw_data_snapshot: 18 campos do Bronze (áreas de gravação, personalização, certificações).
     Elimina 2-3 HTTP calls extras por produto no n8n.';
;
