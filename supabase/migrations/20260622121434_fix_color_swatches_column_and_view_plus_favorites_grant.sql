...
-- ============================================================
-- MIGRATION: fix_color_swatches_column_and_view_plus_favorites_grant
-- APLICADO: 2026-06-22 (via apply_migration)
-- BUG-1: 400 em v_products_public → color_swatches não existe em products nem na view
-- BUG-2: 404 em rpc/get_favorite_list_counts → sem EXECUTE grant para authenticated
-- NOTA: fn_rebuild_color_swatches, fn_trigger_rebuild_swatches_* e triggers OK - apenas faltava a coluna
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- PASSO 1: Adicionar color_swatches a products (causa raiz do 400)
-- ─────────────────────────────────────────────────────────────
ALTER TABLE products
  ADD COLUMN IF NOT EXISTS color_swatches jsonb DEFAULT '[]'::jsonb;

COMMENT ON COLUMN products.color_swatches IS
  'Array JSONB com swatches de cor. [{variant_id,sku,color_id,color_name,color_hex,stock_quantity,image_url,is_in_stock}]. Populado por fn_rebuild_color_swatches via trg_rebuild_swatches_on_variant/image.';

-- ─────────────────────────────────────────────────────────────
-- PASSO 2: Recriar v_products_public com color_swatches no SELECT
-- Reproduzindo exatamente a definição atual + p.color_swatches ao final
-- ─────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────
-- PASSO 3: BUG-2 – Grant EXECUTE em get_favorite_list_counts
-- Função existe (SECURITY DEFINER, usa auth.uid()) mas sem grant → 404
-- ─────────────────────────────────────────────────────────────
GRANT EXECUTE ON FUNCTION get_favorite_list_counts() TO authenticated;

-- ─────────────────────────────────────────────────────────────
-- PASSO 4: Recarregar schema PostgREST
-- ─────────────────────────────────────────────────────────────
