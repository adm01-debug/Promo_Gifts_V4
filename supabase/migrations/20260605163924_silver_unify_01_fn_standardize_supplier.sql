-- UNIFICAÇÃO MEDALLION — Fase 1/4: orquestrador Bronze→Silver em lote.
...
COMMENT ON FUNCTION public.fn_standardize_supplier(uuid, integer) IS
  'Pipeline Medallion (Fase 1->2): Bronze->Silver de-para em lote por fornecedor. Padroniza variantes (fn_standardize_variant) e depois os pais (fn_standardize_parent). Par simetrico de fn_promote_supplier (Fase 2->3). Idempotente; advisory lock por fornecedor.';;
