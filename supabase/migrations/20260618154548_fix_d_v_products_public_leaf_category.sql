...
-- FIX BUG-D (2026-06-18): expor leaf_category_id/name em v_products_public
-- Adiciona LEFT JOIN em mv_product_leaf_category (índice único → index scan puro).
-- As colunas leaf_category_* ficam no FINAL da view para não quebrar ordem existente.
...
SELECT
  ...
  -- FIX BUG-D (2026-06-18): leaf category pré-computada via mv_product_leaf_category
  -- Elimina useProductLeafCategories N+1 (batched GETs com URLs > 11 KB)
...
-- Forçar reload do schema PostgREST para expor as novas colunas na API
