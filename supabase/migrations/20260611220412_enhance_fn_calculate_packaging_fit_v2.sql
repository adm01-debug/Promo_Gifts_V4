...
DECLARE
    ...
    v_max       DECIMAL := 50;
BEGIN
    -- Determinar prefixo de configuração (fragile, precision, bottles ou vazio = default)
    ...
    -- Carregar limites do packaging_compatibility_config (com fallback para default)
    SELECT COALESCE(
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = v_prefix || 'min_gap_mm'),
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = 'min_gap_mm'),
        2
    ) INTO v_min;

    SELECT COALESCE(
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = v_prefix || 'tight_max_mm'),
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = 'tight_gap_max_mm'),
        5
    ) INTO v_tight;

    SELECT COALESCE(
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = v_prefix || 'good_max_mm'),
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = 'good_gap_max_mm'),
        15
    ) INTO v_good;

    SELECT COALESCE(
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = v_prefix || 'max_gap_mm'),
        (SELECT config_value::DECIMAL FROM packaging_compatibility_config WHERE config_key = 'max_gap_mm'),
        50
    ) INTO v_max;
    ...
    -- Calcular folgas (dimensões em cm → folga em mm: × 10)
    IF v_product.shape_type = 'cylindrical' THEN
        v_gap_d := (COALESCE(v_packaging.internal_diameter_cm, v_packaging.internal_width_cm)
                    - COALESCE(v_product.diameter_cm, v_product.width_cm)) * 10;
...
'Calcula compatibilidade dimensional produto×embalagem.
p_config_type: default | fragile | precision | bottles
Retorna jsonb com fit_rating: tight|good|loose|too_tight|too_large|incompatible|error.
Usa packaging_compatibility_config para os limites de folga.';
;
