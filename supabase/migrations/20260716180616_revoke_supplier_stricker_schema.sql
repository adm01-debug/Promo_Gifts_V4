...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'v REVOKE USAGE ON SCHEMA supplier_stricker FROM anon, authenticated';
  ELSE
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'v anon: no USAGE on supplier_stricker (or schema absent)';
  END IF;

  IF has_auth_usage THEN
    RAISE WARNING 'authenticated still has USAGE on supplier_stricker -- check if schema exists';
  ELSE
    RAISE NOTICE 'v authenticated: no USAGE on supplier_stricker (or schema absent)';
  END IF;
END;
$$;;
