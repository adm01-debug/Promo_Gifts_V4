-- ITEM 1 — Dimensões: relocar o parser para a promoção (Silver→Gold) e aposentar o trigger Bronze.
-- fn_promote_padronizacao reproduzida EXATAMENTE (estado pós-#679) + 1 chamada a
-- fn_sync_single_product_dimensions no bloco de raw. Depois: DROP do trigger Bronze.
...
BEGIN
  ...
  IF s.raw_id IS NOT NULL THEN
    UPDATE public.supplier_products_raw
     ...
     WHERE id = s.raw_id;
    -- ITEM 1 (Fase 8, 2026-06-06): dimensões estruturadas na fase de promoção
    -- (substitui o trigger Bronze trg_auto_sync_product_dimensions). Reusa o parser testado;
    -- idempotente e com EXCEPTION interna, não interfere na promoção.
...
-- Aposenta o trigger Bronze (agora redundante: dimensões via promoção)
