...
-- Recriar v_products_public para referenciar a nova MV (o DROP CASCADE já a derrubou)
...
SELECT
  ...
  -- FIX BUG-D v2 (2026-06-18): COALESCE com is_primary-first MV
  COALESCE(lc.leaf_category_id, p.main_category_id, p.category_id) AS leaf_category_id,
  COALESCE(lc.leaf_category_name, NULL)                             AS leaf_category_name,
  lc.leaf_category_level                                            AS leaf_category_level,
  lc.leaf_category_slug                                             AS leaf_category_slug
