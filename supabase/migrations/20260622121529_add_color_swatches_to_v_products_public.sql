...
-- APLICADO 2026-06-22
-- Adiciona color_swatches à v_products_public para RPC get_catalog_bestseller_page
-- e queries diretas via view. CREATE OR REPLACE preserva grants e dependências.
...
SELECT
    ...
    p.is_featured, p.is_new, p.is_on_sale,
    ...
    p.supplier_reference, p.is_textil, p.has_capacity, p.combined_sizes,
    p.gender, p.is_stockout, p.is_online_exclusive, p.catalog_page,
    ...
    p.diameter_cm, p.shape_type,
    p.internal_height_cm, p.internal_width_cm, p.internal_length_cm,
    p.internal_diameter_cm, p.packaging_material, p.packaging_color,
    p.has_inner_cradle, p.cradle_material, p.packaging_finish,
    p.is_imported, p.lead_time_days, p.requires_minimum_order,
    p.supply_mode, p.is_thermal, p.capacity_ml, p.slug,
    p.ai_summary, p.key_benefits, p.use_cases, p.target_audience,
    p.schema_json, p.canonical_url, p.robots_meta, p.seo_score,
    p.seo_last_audit_at, p.seo_issues, p.og_title, p.og_description,
    p.og_image_url, p.description_packaging_info, p.has_optional_packaging,
    p.optional_packaging_ref, p.packing_classification,
    NULL::numeric AS ipi_rate,
    NULL::character varying AS tax_reference_state,
    p.engraving_type, p.supplier_updated_at, p.has_gift_box,
    p.min_order_quantity, p.ai_title, p.ai_description, p.ai_version,
    p.ai_generated_at, p.ai_model, p.box_image, p.repacking_classification,
    p.has_commercial_packaging, p.packaging_context,
    ...
    p.freight_class, p.cubic_weight,
    ...
    -- 2026-06-22: COLOR SWATCHES V2 — adicionado à view para expor via PostgREST e RPC
    -- Populado por fn_rebuild_color_swatches (P1→P4 fallback). 7.153 produtos, 16.631 swatches.
    -- Consumido por: get_catalog_bestseller_page RPC + PRODUCT_SELECT_LIGHTWEIGHT + ColorSwatchPicker.
    p.color_swatches
FROM products p
LEFT JOIN mv_product_leaf_category lc ON (lc.product_id = p.id)
WHERE (p.is_deleted IS NOT TRUE) AND (p.is_active = true);

-- PostgREST: invalidar cache para reconhecer nova coluna
