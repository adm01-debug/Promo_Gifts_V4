-- Forward-only: deduplicação de auditoria passa a usar exclusivamente o ID da
-- solicitação. Registros legados sem correlação podem ganhar um único backfill,
-- mas nunca suprimem uma solicitação distinta por proximidade temporal.
...
BEGIN
  IF position('_responsible_id := COALESCE(_q.seller_id, _q.created_by)' IN _def) = 0
     OR position('aal.created_at >=' IN _def) > 0
     OR position('aal.request_id = _result.id::text' IN _def) = 0 THEN
    RAISE EXCEPTION 'Postcondition failed: correlação exata de audit não persistida';
  END IF;
END
$post$;
