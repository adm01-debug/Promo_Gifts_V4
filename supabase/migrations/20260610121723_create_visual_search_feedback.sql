...
-- A tabela nunca existiu no banco vivo → o insert de feedback falhava.
-- product_id é text (sem FK): produtos vivem no BD externo (Promobrind), não local.
...
-- Insert: qualquer usuário autenticado registra feedback (próprio uid ou anônimo nulo).
CREATE POLICY vsf_insert_authenticated ON public.visual_search_feedback
  FOR INSERT TO authenticated
  WITH CHECK (user_id IS NULL OR user_id = (SELECT auth.uid()));

-- Leitura/curadoria: dono do registro ou admin (feedback é insumo de melhoria da IA).
