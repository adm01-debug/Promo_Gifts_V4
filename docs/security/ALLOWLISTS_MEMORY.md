...
- **Edição Magazine v2** (`magazine_create_v2`, `magazine_update_metadata_v2`, `magazine_add_items_v2`, `magazine_remove_items_v2`, `magazine_reorder_items_v2`, `magazine_update_item_v2`, `magazine_duplicate_v2`, `magazine_publish_v2`, `magazine_unpublish_v2`, `magazine_archive_v2`, `magazine_reactivate_v2`, `magazine_soft_delete_v2`, `magazine_restore_v2`, `magazine_import_local_v2`) — exige `auth.uid()`, owner/admin, estado compatível e CAS monotônico por `edit_version`; criação/importação validam organização, payload e idempotência; `anon`/`PUBLIC` permanecem revogados.
...
Snapshot canônico reconciliado em 2026-09-09: **11 entradas**, todas listadas literalmente
...
- `public.get_sitemap_public(p_limit integer, p_offset integer)` — Endpoint SEO público read-only confirmado no canônico em 2026-09-09: retorna somente revistas publicadas, não apagadas e com token público; projeção fixa, paginação limitada e `search_path` fixo.
...
- `public.magazine_add_items_v2(p_magazine_id uuid, p_expected_edit_version bigint, p_items jsonb)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, rascunho, lock, CAS monotônico e itens válidos/deduplicados; `anon`/`PUBLIC` revogados.
- `public.magazine_archive_v2(p_magazine_id uuid, p_expected_edit_version bigint)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, lock, CAS e transição de estado válida; `anon`/`PUBLIC` revogados.
- `public.magazine_create_v2(p_organization_id uuid, p_title text, p_template_id text)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, valida organização e cria rascunho pertencente ao ator; `anon`/`PUBLIC` revogados.
...
- `public.magazine_duplicate_v2(p_source_magazine_id uuid, p_expected_edit_version bigint, p_idempotency_key text, p_title text)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, CAS e idempotência vinculada a ator/payload; a cópia nasce como rascunho; `anon`/`PUBLIC` revogados.
- `public.magazine_import_local_v2(p_idempotency_key text, p_payload jsonb)` — Importação v2 confirmada no canônico em 2026-09-09: exige ator autenticado, valida limites/payload e garante idempotência por ator; `anon`/`PUBLIC` revogados.
...
- `public.magazine_publish_v2(p_magazine_id uuid, p_expected_edit_version bigint)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, lock, CAS e validação atômica de conteúdo/páginas; `anon`/`PUBLIC` revogados.
- `public.magazine_reactivate_v2(p_magazine_id uuid, p_expected_edit_version bigint)` — RPC v2 confirmada no canônico em 2026-09-09: reativa somente estado permitido para rascunho, sob autorização, lock e CAS; `anon`/`PUBLIC` revogados.
...
- `public.magazine_remove_items_v2(p_magazine_id uuid, p_expected_edit_version bigint, p_item_ids uuid[])` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, rascunho, lock e CAS; restringe a remoção à revista; `anon`/`PUBLIC` revogados.
...
- `public.magazine_reorder_items_v2(p_magazine_id uuid, p_expected_edit_version bigint, p_ordered_item_ids uuid[])` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, rascunho, lock e CAS; valida permutação completa sem IDs externos/duplicados; `anon`/`PUBLIC` revogados.
- `public.magazine_restore_v2(p_magazine_id uuid, p_expected_edit_version bigint)` — RPC v2 confirmada no canônico em 2026-09-09: restaura exclusão lógica para rascunho sob autorização, lock e CAS; `anon`/`PUBLIC` revogados.
- `public.magazine_soft_delete_v2(p_magazine_id uuid, p_expected_edit_version bigint)` — RPC v2 confirmada no canônico em 2026-09-09: realiza exclusão lógica, sem apagar dados, sob autorização, lock e CAS; `anon`/`PUBLIC` revogados.
- `public.magazine_unpublish_v2(p_magazine_id uuid, p_expected_edit_version bigint)` — RPC v2 confirmada no canônico em 2026-09-09: retira publicação atomicamente sob autorização, lock e CAS; `anon`/`PUBLIC` revogados.
- `public.magazine_update_item_v2(p_magazine_id uuid, p_expected_edit_version bigint, p_item_id uuid, p_patch jsonb)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, rascunho, lock, CAS, patch allowlisted e item da revista; `anon`/`PUBLIC` revogados.
...
- `public.magazine_update_metadata_v2(p_magazine_id uuid, p_expected_edit_version bigint, p_patch jsonb)` — RPC v2 confirmada no canônico em 2026-09-09: exige ator autenticado, owner/admin, rascunho, lock, CAS e patch allowlisted/null-safe; `anon`/`PUBLIC` revogados.
...
- `public.get_sitemap_public(p_limit integer, p_offset integer)` — Sitemap público read-only: projeção SEO fixa de revistas publicadas, não apagadas e com token; paginação limitada a 1.000, `search_path` fixo e views subjacentes sem `SELECT` para `anon`.
