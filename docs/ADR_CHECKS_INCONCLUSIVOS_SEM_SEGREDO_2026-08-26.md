...
- **Status:** proposto (somente análise; nenhuma automação foi alterada)
- **Data:** 2026-08-26
- **Escopo:** etapas 92 e 94 do plano de melhorias: smoke/integrations reais e semântica de checks sem segredo.
- **Não escopo:** não foram executados requests remotos, migrations, DDL, deploys, cargas, webhooks ou testes contra produção. Não há alteração de schema, dados, secrets ou workflows nesta ADR.
...
|---|---|---|---|---|
| **ACL SECURITY DEFINER** — `scripts/check-security-definer-acl.mjs`; `.github/workflows/magazine-unit-tests.yml`; `.github/workflows/security-definer-acl-multi-env.yml`; `.github/workflows/freight-quality-gates.yml` | Sem URL/chave, o script escreve `skip` e termina com `exit 0`. Em `magazine-unit-tests.yml`, o job aparece como dependência do resumo `Magazine Tests OK`; no workflow multi-ambiente, os passos são condicionais, mas o job termina com sucesso. | Um check verde pode não ter consultado `audit_security_definer_acl()` e ainda ser agregado como qualidade/security aprovada. | Adicionar modo `--require-live`/`REQUIRE_LIVE=1` que resulte em `inconclusive` quando faltarem credenciais; emitir JSON com URL mascarada, request count e resultado. Não incluir jobs sem evidência no summary `OK`. | **Sim**, para provisionar segredo de leitura no Environment de staging e definir se o gate é obrigatório em PR/fork. A alteração de código por si só não exige acesso externo. |
| **Drift de lints 0011, 0029 e EXECUTE para anon** — `scripts/check-lint-0011-drift.mjs`, `scripts/check-lint-0029-drift.mjs`, `scripts/check-secdef-anon-drift.mjs`; `.github/workflows/ci.yml` | Os três retornam `null` tanto sem credencial como em HTTP não-OK do `pg-meta`, imprimem `skip` e usam `exit 0`. O quality gate principal os chama com secrets que podem estar ausentes. | Ausência, token inválido, endpoint indisponível ou permissão insuficiente fica indistinguível de “nenhum drift”. | Distinguir `missing-config`, `http-error` e `no-findings`; falhar/ficar inconclusivo nos dois primeiros. Aceitar `--from-file` somente como evidência estática explicitamente rotulada. | **Sim**, para uma credencial de leitura `pg-meta` no alvo não produtivo; decisão do PO sobre obrigatoriedade por tipo de evento. |
...
| **Gate de RPC `get_profile_and_roles`** — `scripts/check-rpc-permissions.mjs`; `scripts/check-rpc-get-profile-and-roles.mjs`; `.github/workflows/supabase-security-gate.yml` | Embora ambos falhem quando o env inicial falta, erros/HTTP não-OK ao consultar `information_schema` são tratados como `✅ pulada`; no segundo, o smoke inacessível também fecha `0` com mensagem de audit suficiente, mesmo se o audit não trouxe dado. | Perda de acesso de leitura pode ser apresentada como permissões/RPC confirmadas. | Somente aprovar após resposta verificável do endpoint de auditoria e do teste de existência; erro de leitura/rede deve ser `inconclusive`. | **Sim**, para uma rota de auditoria/credencial de leitura estável no ambiente de teste. |
...
## Plano mínimo de implementação posterior (não executado nesta ADR)

1. Criar uma pequena convenção comum de resultado (`status`, `mode`, `target`, `requests`, `tests_executed`, `tests_skipped`, `reason`) para scripts remotos.
...
5. Migrar primeiro os gates de ACL/lint/schema/RPC, pois alegam verificação de estado vivo do banco.
