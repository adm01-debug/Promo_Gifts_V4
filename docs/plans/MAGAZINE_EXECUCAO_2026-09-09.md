...
**Três rodadas locais implementadas e testadas; o plano de 50 etapas NÃO está concluído.** A terceira rodada implementa o modelo editorial estruturado decidido pelo PO e prepara cinco RPCs transacionais forward-only. O checklist de homologação integral continua aberto. Não há base para declarar 10/10, fidelidade pixel a pixel às cinco imagens ou produção atualizada.

A instrução explícita do PO autorizou páginas estruturadas (`capa`, `institucional`, `seção`, `produtos`, `contato`), reutilização dos assets atuais e **preparação sem aplicação** das RPCs atômicas. O Supabase canônico não sofreu alterações.
...
10. Foram preparados, mas não aplicados, cinco RPCs com lock, ownership/admin, CAS, limite de lote, `search_path` fixo e privilégio mínimo: adicionar/remover/reordenar itens, duplicar revista e atualizar metadados. A integração do cliente permanece deliberadamente desligada até aprovação/aplicação coordenada.
...
| 011   | Preparado/testado, não aplicado | Cinco migrations forward-only definem RPCs atômicas, CAS e privilégios mínimos. Contratos estáticos e dry-run PostgreSQL 17 passam; falta autorização de aplicação.                                                                       |
| 012   | Preparado, não aplicado         | RPCs de add/remove/reorder fazem uma transação por chamada e bloqueiam a revista; runtime atual continua no caminho legado até rollout aprovado.                                                                                          |
...
| 042   | Implementado/testado local      | Envelope versionado em JSONB reutiliza `page_order`; arrays/null legados permanecem automáticos, sem migration de dados.                                                                                                                  |
...
| 044   | Implementado/testado local      | Reordenação preserva capa/contato nas extremidades; itens novos não atribuídos entram antes do contato; IDs duplicados/malformados são rejeitados.                                                                                        |
...
| 049   | Preparado, não aplicado         | Cinco objetos foram autorizados apenas para preparação. Nenhuma migration/deploy foi aplicada no canônico; rollout ainda exige autorização explícita.                                                                                     |
| 050   | Pendente                        | Mudanças locais isoladas; sem merge, push ou publicação nesta rodada; homologação integral não encerrada.                                                                                                                                 |

## Próxima sequência segura

1. Revisar o resultado do dry-run e o plano de rollout/rollback; solicitar autorização explícita antes de qualquer aplicação canônica.
...
3. Homologar as páginas estruturadas no shell completo e decidir se os assets aproximados são suficientes ou se haverá fornecimento de fotografias finais.
