...
**Método inicial:** simulações locais e consultas somente leitura no banco. **Atualização posterior:** reviewer de Production e secret GitHub `PGHOST` alterados com autorização; primeira aplicação E15 falhou antes da DDL. Não houve DDL, reparo de ledger, exclusão nem deploy de Edge nesta sequência até a coleta descrita abaixo.
...
**Decisão posterior do PO:** a proposta `20260922170000` foi explicitamente aprovada e promovida ao diretório de migrations, sem mudança no SQL executável. PRs #1872 e #1873 mergeadas. Reviewer `adm01-debug` configurado após aprovação específica. O preflight da segunda run passou e a aplicação canônica aguarda aprovação humana em Production; ver [pacote de aplicação](../db/APLICACAO_SIGNUP_20260922170000.md). Aprovação da migration não é recibo de execução.

## Continuação operacional: correção de pooler (22/09, após 17:13 UTC)
