...
**Estado atualizado, 22/09 às 21:00 UTC:** dez etapas integralmente verificadas; as demais seguem parciais ou bloqueadas. O estado global permanece **NÃO ALINHADO**, mas **a correção de `handle_new_user()` está aplicada e comprovada**. Recibo signup e gate de tipos estão na main pelos PRs #1875/#1876. Contratos de variante/personalização foram mergeados no PR #1878 (`796a52f07`), com 281 testes locais aprovados. O PO autorizou preparar/simular as duas RPCs: propostas fora de migrations, 41 verificações PostgreSQL e seis testes do pacote aprovados, **sem aplicação**. A edição ainda depende de decisão sobre trigger de versão e remoção explícita; não liberar a candidata atual. Ver [execução](EXECUCAO_RECONCILIACAO_50_ETAPAS_2026-09-22.md) e [relatório de simulação/hashes](../db/RELATORIO_SIMULACAO_QUOTE_RPCS_2026-09-22.md). Nenhuma DDL no canônico nesta rodada.
...
- [ ] **37. Reconciliar `quote_items.product_variant_id`** `[GIT/BD, P1]` — tipos/FKs gerados e catálogo verificados; identidade propagada no domínio/payload/duplicação e SKU sem ambiguidade. **Pendente:** `create_quote_transactional` e `update_quote_transactional` não gravam a coluna no corpo vivo. **Aceite:** compilar e passar contratos locais, além de preservar variante em criar/ler/editar/duplicar no PostgreSQL; alterações de funções dependem de aprovação específica.
