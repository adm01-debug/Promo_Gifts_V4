...
fixture.id='audit-magazine'; fixture.ownerId='audit-owner';fixture.title='Soluções que aproximam';
fixture.subtitle='Descrição exclusiva auditável';fixture.branding.clientName='Alfa Cliente';
window.__products=fixture.items.map((it,i)=>({...it.productSnapshot,primary_image_url:it.productSnapshot.image_url,category_name:i%2?'Escritório':'Tecnologia'}));
let stored=structuredClone(fixture);
window.__calls=[];window.__stored=()=>structuredClone(stored);window.__delay=0;
window.__service={
  create:async(input)=>{window.__calls.push({method:'create',input});stored={...structuredClone(fixture),id:'created-audit',items:[],templateId:input.templateId};return structuredClone(stored)},
  list: async()=>Array.from({length:8},(_,i)=>({...structuredClone(stored),id:i===0?stored.id:`audit-${i}`,status:i<3?'draft':i<7?'published':'archived',templateId:listTemplates()[i].id,viewCount:i*100})),
...
window.__reset=(empty=false)=>{stored=structuredClone(fixture);if(empty)stored.items=[];window.__calls=[]};
window.__registry=listTemplates().map(({Component,...entry})=>entry);
function Controls(){window.__navigate=useNavigate();return null;}
createRoot(document.getElementById('root')!).render(<HelmetProvider><QueryClientProvider client={new QueryClient({defaultOptions:{queries:{retry:false}}})}><TooltipProvider><BrowserRouter><Controls/><React.Suspense fallback={<p>Carregando...</p>}><Routes><Route path="/magazine" element={<MagazineListPage/>}/><Route path="/magazine/templates" element={<Gallery/>}/><Route path="/magazine/:id" element={<MagazineEditorPage/>}/></Routes></React.Suspense><Toaster/></BrowserRouter></TooltipProvider></QueryClientProvider></HelmetProvider>);
